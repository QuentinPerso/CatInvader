...
public class Pnjview extends CharacterView {
	private Pnj pnj;
	...
	Pnjview (final View view, Pnj pnj) {
		super(view);
		this.pnj = pnj;
	...
	void render (SpriteBatch batch) {
		...
		if(pnj.type==pnjType.kuro){
			...
			float bodywidth  = Pnj.width +  1f;
			float bodyheight = Pnj.height+ .4f*1.6f;
			...
			float weapx = pnj.position.x+Pnj.width+0.3f-weapwidth/3;
	...
	void shoot () {
		...
		pnj.shootTimer = 1/Pnj.shootpsec;
		burstTimer = Pnj.burstDuration;
		
		// Compute the position and velocity to spawn a new bullet.
		weaponx = pnj.position.x+Player.width+0.3f;
		...
		float vx = cos * RGbulletSpeed;// + pnj.velocity.x * RGbulletInheritVelocity;
		...
		burstShots = Math.min(20, burstShots + 1);
	}
	void dispose(){
		shoots.dispose();
	}
